...
2. A **pod** is a compartment inside the organization. Every org starts with a Default Pod, which cannot be deleted. Add more pods if you're building a [platform that gives email addresses](/advanced/multi-tenant) to your customers' agents.
...
```bash Pod- or inbox-scoped key
curl "https://api.agentmail.to/v0/inboxes" \
  -H "Authorization: Bearer $AGENTMAIL_API_KEY"
```

```bash Organization-scoped key
curl "https://api.agentmail.to/v0/pods/<pod_id>/inboxes" \
  -H "Authorization: Bearer $AGENTMAIL_API_KEY"
```
