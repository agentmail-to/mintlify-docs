...
# Diagnose spam placement, bounces, complaints, and missing mail

Receiving providers decide placement on their own using three signals: authentication, sender reputation, and content. Use this page to understand why mail lands in spam, why sends bounce, what a complaint means, why a domain fails to verify, and why inbound mail goes missing.

## Do this

When outbound mail lands in spam, work through these causes in order, most frequent first:

1. Missing or misconfigured SPF, DKIM, or DMARC records. The most common cause by far.
2. A new domain sending real volume before it has any reputation.
3. Content that looks like spam: HTML with no plain-text version, trigger words like "free" and "urgent", all-caps subjects, stacked links, or images in the body. Open trackers count as images.
4. A bounce rate high enough that providers stop trusting the traffic.
5. Production sending from the shared `@agentmail.to` domain, whose reputation every AgentMail user shares.

When an expected inbound email is missing, check in this order:

1. It arrived but is hidden. Default listings hide messages labeled `spam`, `blocked`, `unauthenticated`, and `trash`. Include flags reveal each label, see [/core/receive](/core/receive#choose-which-messages-you-list).
2. The sender failed authentication under their own enforcing DMARC policy, so the message was dropped. The fix is on the sender's side.
3. The mail never reached AgentMail: the domain's MX records point elsewhere, the domain is not verified yet, or Google Workspace routed mail to a subdomain internally so it never left Workspace.

## Facts

- `message.delivered` means the receiving server took the message. A delivered message can still land in spam. The provider then chooses between inbox and spam using authentication, reputation, and content.
- SPF is a TXT record listing the servers allowed to send for a domain. A host can carry only one SPF record. Merge AgentMail's `include:` mechanism into an existing record.
- DKIM signs every outgoing message with a private key AgentMail holds for the domain. Receivers fetch the public key from the selector host, like `agentmail._domainkey.example.com`. A valid signature proves the origin domain and that nothing changed in transit.
- SPF checks the domain in the message's return path. DKIM checks whichever domain signed the message. A message can pass both and still fail DMARC when neither passing identity aligns with the visible `From` domain.
- DMARC policies: `p=none` asks receivers to take no action, `p=quarantine` asks receivers to send failing mail to spam, `p=reject` asks receivers to refuse it outright. AgentMail generates a `reject` record. The `rua` tag names the address for aggregate authentication reports.
- Gmail, Outlook, Yahoo, and the other major providers expect all three records for reliable inbox placement. Shared `@agentmail.to` addresses have all of this in place already.
- Publishing the records grants AgentMail exactly two things: permission to send as the domain and delivery of the domain's inbound mail. The website and the rest of the DNS are untouched.
- MX records decide where a domain's inbound mail is delivered. The TXT records only govern how outbound mail is judged. AgentMail can send from a domain whose inbound mail stays with Gmail.
- Sender reputation is each provider's private per-domain score, built from authentication results on past mail, delivery outcomes (above all the bounce rate), recipient behavior (opens, replies, spam complaints), and sending consistency.
- A new domain starts at zero and zero is not neutral. Providers treat sudden volume from a new domain as hostile. Replies to inbound mail build reputation fastest because two-way conversation is the strongest engagement signal.
- Reputation is scored per domain. Outreach split across separately registered subdomains lets a flagged domain be cycled out without touching the others.
- Permanent bounces mean the address is unreachable for good: the address does not exist, its domain does not exist, or the mailbox was deleted. Transient bounces are temporary: a full mailbox, a briefly down server, a message too large, or greylisting.
- AgentMail retries transient bounces automatically and treats a message that keeps failing as permanently bounced.
- The `message.bounced` event reports the provider's classification in its `bounce` object: a `type` and `sub_type` plus the affected recipients.
- One recipient bouncing is a data problem, correct or drop the address. Bounces across many recipients or receiving domains are a sending problem, check authentication and the recent sending pattern before sending more.
- After a hard bounce, a spam complaint, or an unsubscribe, AgentMail stops future sends to that address automatically. The suppression entry is read-only. Email support@agentmail.cc if an entry genuinely needs review.
- A complaint means the message was delivered and the recipient marked it as spam through their provider's feedback loop. AgentMail surfaces it as a `message.complained` event and suppresses the address like a hard bounce. Providers tolerate far fewer complaints than bounces.
- Domain verification is AgentMail querying public DNS for the records it generated. A DNS change takes from a minute to 48 hours to become visible, most are live within half an hour.
- DNS records only count at the provider that controls the domain's active nameservers. Common entry mistakes: the domain appended to the host twice, extra quotes around a TXT value, stray spaces, or a conflicting record (a second SPF record or a stale DKIM key) on the same host.
- AgentMail rechecks verified domains about once a day. A domain whose records are later edited or deleted reverts to unverified, which blocks new inbox creation until the records are restored.
- AgentMail checks SPF, DKIM, and DMARC on every inbound message. Mail carrying a virus, and mail that fails DMARC while the sender's own policy is `quarantine` or `reject`, is dropped before delivery and never stored.
- Inbound mail that fails authentication under a permissive policy, or arrives with no authentication headers, is delivered with the `unauthenticated` label. Subscribe to the `message.received.unauthenticated` event to watch it arrive.
- Auto-forwarded mail often fails direct SPF and DKIM checks at the final hop. AgentMail then validates the message's ARC chain. If the chain verifies and the sealer is microsoft.com, google.com, outlook.com, or icloud.com, the original sender's authentication stands and forwarding keeps working.

## Not supported

- `message.delivered` does not mean the message reached the inbox folder.
- Passing SPF, DKIM, and DMARC does not guarantee inbox placement. Authentication proves who sent the mail, reputation decides how much that sender is trusted.
- No DNS record or account setting controls reputation by itself. Only sending differently changes it.
- Retrying a permanent bounce cannot succeed, and repeated sends to dead addresses damage reputation quickly.
- A complaint has nothing to retry. Transport succeeded and the recipient rejected the mail itself.
- Full authentication does not offset a complaint. An authenticated message that recipients report is still spam in the provider's eyes.
- Automatic suppression entries cannot be cleared by the sender. Correct the cause instead.
- Two SPF records on one host fail authentication outright.
- Inbound mail dropped for a virus or an enforcing DMARC failure produces no bounce. The sending server can even report a successful delivery, because AgentMail accepted the message before rejecting it during verification.
- Records added at a DNS provider that does not control the domain's active nameservers are invisible to public DNS and do not count toward verification.

## Related

- [/advanced/deliverability](/advanced/deliverability) - the warmup ramp, monitoring thresholds, and metrics endpoint built on these mechanics.
- [/advanced/custom-domains](/advanced/custom-domains) - register a domain, publish the records, fix verification, and check where a domain's mail goes.
- [/core/receive](/core/receive) - include flags that reveal messages hidden by the `spam`, `blocked`, `unauthenticated`, and `trash` labels.
