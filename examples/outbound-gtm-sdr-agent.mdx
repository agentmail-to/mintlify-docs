...
# Build an outbound SDR agent on Vercel Eve and AgentMail

Builds an agent that loads leads, researches every prospect, sends personalized first touches from its own inbox, follows up on silence, classifies replies with a small model, and hands hot leads to a human with the thread and research attached. Use it to run cold outbound with a human approval gate on every send.
...
Eve reads `agent/instructions.md` as the system prompt, each file in `agent/tools/` as a tool, and `agent/channels/` as HTTP entry points. `npm run dev` opens an interactive session with the agent.

## SDK

`npm install agentmail`. The page's client is TypeScript: `new AgentMailClient()` reads `AGENTMAIL_API_KEY`.

- Add the outreach domain: `agentmail.domains.create({ domain: "tryacme.com" })`. Also CLI `agentmail domains create --domain tryacme.com`, HTTP `POST https://api.agentmail.to/v0/domains`, Python `client.domains.create(domain="tryacme.com")`.
- Verify the domain: `agentmail.domains.verify(domain.domainId)`, then wait until `status` is `VERIFIED`.
- Create the inbox: `agentmail.inboxes.create({ username: "mara", domain: "tryacme.com", displayName: "Mara Ellison" })`. The response's `email` field is the agent's address. Set it as `SDR_INBOX`.
- Send the first touch: `agentmail.inboxes.messages.send(INBOX, { to: [to], subject, text }, { headers: { "Idempotency-Key": ... } })`.
- Schedule the follow-up: `agentmail.inboxes.drafts.create(INBOX, { inReplyTo: messageId, text, sendAt })` with `sendAt` an ISO timestamp 4 days out.
- Cancel the follow-up: `agentmail.inboxes.drafts.delete(INBOX, draftId)`.
- Register the webhook: `agentmail.webhooks.create({ url, eventTypes: ["message.received", "message.bounced", "message.complained"], inboxIds: [INBOX], clientId: "sdr-agent-v1" })`.
- Reply and forward: thin wrappers on the reply and forward endpoints, each send carrying an `Idempotency-Key` header.
- Label a thread handed off: `agentmail.threads.update(threadId, { addLabels: ["handed-off"] })`.
- Block a sender org-wide: `agentmail.lists.create("send", "block", { entry })`.

Full SDK surface: [/integrations/sdks-and-cli](/integrations/sdks-and-cli).

## Facts

- Custom domains require the Developer plan or above. The Free plan does not include them.
- Requires Node.js 24 or newer and an AI Gateway key (or any model provider key) for Eve.
- Use a registered outreach domain that is not the main domain. Let it age about two weeks before sending.
- Env vars: `AGENTMAIL_API_KEY`, `SDR_INBOX` (the inbox address), `AGENTMAIL_WEBHOOK_SECRET` (the Svix secret, shown once at webhook creation), `EXA_API_KEY` (research tool).
- `domains.create` returns the SPF, DKIM, DMARC, and MX records to publish at the registrar. Publish them exactly as returned. If the domain already has an SPF record, merge the new value into it instead of adding a second record.
- Create inboxes on a domain only after `status` is `VERIFIED` (or after the `domain.verified` webhook event).
- Name the inbox for a person. `mara@tryacme.com` gets more replies than `sales@tryacme.com`.
- Hard cap: 25 sends per inbox per day, the deliverability ceiling for one warmed inbox.
- Every send carries an `Idempotency-Key` header, so a crash and retry sends one email, not two.
- The follow-up is a scheduled reply draft. `inReplyTo` points at the first touch, so recipients, subject, and threading derive from it, and `sendAt` sets the date. Canceling is one draft delete, with no separate scheduler to race.
...
- Reply categories: `interested`, `question`, `not_now`, `not_interested`, `out_of_office`, `referral`, `unsubscribe`, `auto_reply`.
- Unsubscribes, declines, bounces, and complaints go to the send block list and the lead is suppressed for good. `out_of_office` reschedules the follow-up. `auto_reply` gets no action.
- The send block list is org-level. An opt-out binds every future campaign even if a fresh lead list re-imports the address.
- Hard rules: max 3 touches per prospect, nothing after any human reply, no reply to a message whose `Auto-Submitted` header is set, no reply in a thread labeled `handed-off`.
- Every outbound send is gated on human approval (`approval: always()` in Eve) until relaxed per category.
- Warmup runs through an external tool (Instantly, Smartlead) over IMAP/SMTP: IMAP host `imap.agentmail.to` port `993`, SMTP host `smtp.agentmail.to` port `465`, both SSL/TLS, username is the inbox address, password is the API key.
- Keep bounces under 2 percent and complaints under 0.3 percent per campaign. Stop the ramp when `message.bounced` or `message.complained` rates move.

## Not supported

- No cold sends before warmup finishes. A new inbox needs a few weeks of warmup traffic.
- Sends to previously bounced, complained, or unsubscribed addresses do not go out. AgentMail rejects them with `403 MessageRejectedError` before anything leaves.
- Webhook delivery is not exactly once. A handler without message id dedupe processes repeats.

## Errors

| Error | HTTP status | Cause | Fix |
| --- | --- | --- | --- |
| `MessageRejectedError` (code `message_rejected`) | 403 | The recipient previously bounced, complained, or unsubscribed and is on the org send block list | Mark the lead suppressed and stop contacting the address |

## Verify

Run `npm run dev`, hand the agent a few test leads, and watch it research, draft, ask for send approval, and schedule follow-ups. The full pipeline works against a test inbox on `agentmail.to` immediately while the outreach domain warms.

## Related

- [/examples/customer-support-agent](/examples/customer-support-agent) - the inbound counterpart, same webhook and channel pattern.
- [/examples/ai-employee-with-its-own-inbox](/examples/ai-employee-with-its-own-inbox) - an agent teammates email directly.
- [/advanced/custom-domains](/advanced/custom-domains) - adding, publishing DNS for, and verifying the outreach domain.
- [/advanced/deliverability](/advanced/deliverability) - warmup ramp schedules.
- [/advanced/webhooks](/advanced/webhooks) - event payloads and Svix signature verification.
- [/core/imap-smtp](/core/imap-smtp) - the credentials warmup tools connect with.
