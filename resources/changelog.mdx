...
# Track AgentMail platform changes and their dates

This page lists every AgentMail platform change with its release date, newest first. Use it to check whether a feature, endpoint, or behavior existed at a point in time, or to catch changes that replaced, renamed, or removed a surface you depend on.

## 2026-07-20 AgentID public-key API credentials

- Added `POST /v0/api-keys/public-keys` (register a public P-256 JWK, receive the server-owned `api_key_id` used as `kid`), `GET /v0/api-keys/public-keys`, `PATCH /v0/api-keys/public-keys/{api_key_id}` (rename only), `DELETE /v0/api-keys/public-keys/{api_key_id}`, and `POST /v0/api-keys/public-keys/agentid-sign-in/revoke-all` (idempotently invalidates every current AgentID sign-in key in an organization).
- Added `POST https://auth.agentid.com/authorize/approve` on the AgentID issuer for one strict ES256 approval assertion without a bearer credential. Served by the AgentID issuer, not part of the AgentMail REST API or generated SDKs.
- Public-key credentials can be organization-, pod-, or inbox-scoped with inherited scope and expiry defaults. Private key material stays in your own keystore.

## 2026-06-25 Reply and forward drafts

- `inboxes.drafts.create` accepts `in_reply_to` (reply to the sender, add `reply_all` to address the whole thread) and `forward_of` (forward the message, recipients caller-supplied). The two parameters are mutually exclusive, and reading the referenced message requires the `message_read` permission.
- Replies carry over recipients, subject, and threading (`in_reply_to` / `references`) from the original. Forwards carry over subject, threading, and the forwarded body and attachments (merged in at send time), with recipients optional so the draft can be addressed later.
- Draft responses now include `forward_of`, the ID of the message a forward draft was created from. Combine with `send_at` to schedule, or send later with `inboxes.drafts.send`.

## 2026-06-25 Pod- and inbox-scoped webhooks

- Added `GET|POST /v0/pods/:pod_id/webhooks`, `GET|PATCH|DELETE /v0/pods/:pod_id/webhooks/:webhook_id`, `GET|POST /v0/inboxes/:inbox_id/webhooks`, and `GET|PATCH|DELETE /v0/inboxes/:inbox_id/webhooks/:webhook_id`. Scope comes from the path, so pod- and inbox-scoped API keys can manage their own webhooks.
- A pod-scoped webhook receives events for the whole pod and can be narrowed with `inbox_ids`. `pod_ids` is not passed. An inbox-scoped webhook is fixed to that inbox, only its `event_types` can be changed.
- A scoped webhook must always keep at least one pod or inbox subscription.

## 2026-06-12 Usage metrics and the events path

- Added `GET /v0/metrics/usage`, `GET /v0/pods/:pod_id/metrics/usage`, and `GET /v0/inboxes/:inbox_id/metrics/usage`. Each point is the running total at that timestamp. Usage types: `storage_bytes`, `message_count`, `thread_count`, `inbox_count`, `domain_count`, `pod_count` (inboxes carry the first three, pods add `inbox_count` and `domain_count`, organizations add `pod_count`). Filter with `usage_types`. Range divided by `period` (bucket size in seconds) must not exceed 1000 buckets.
- Event counts moved to `GET /v0/metrics/events` (and pod/inbox variants), replacing bare `GET /v0/metrics`. The old path still responds but is removed from docs and SDKs. Use `client.metrics.queryEvents()` in place of `client.metrics.query()`.
- Documented event types now match the API: added `message.received.spam`, `message.received.blocked`, `message.received.unauthenticated`, `domain.verified`. Removed `message.delayed`, which the API never accepted. A future `end` clamps to the current time, and `period`/`limit` must be whole numbers.

## 2026-06-08 Subdomains on verified domains

- `POST /v0/domains` accepts optional `subdomains_enabled` (defaults to `false`). When enabled, verification records include a wildcard MX record (`*.<domain>`) to publish on the top-level domain. Once verified, inboxes can be created on any subdomain of that domain.
- `PATCH /v0/domains/:domain_id` accepts `subdomains_enabled` with partial updates: send at least one of `feedback_enabled` or `subdomains_enabled`, omitted fields are unchanged. Enabling on an already-verified domain returns it to `pending` until the wildcard MX record is published. Sending is not interrupted. Domain responses now include `subdomains_enabled`.
- Creating an inbox on a subdomain of a domain without subdomains enabled returns a `422` error.

## 2026-06-03 Full-text search for messages and threads

- Added `GET /v0/inboxes/:inbox_id/messages/search`, `GET /v0/threads/search` (org-wide), `GET /v0/inboxes/:inbox_id/threads/search`, and `GET /v0/pods/:pod_id/threads/search`. A `q` query matches sender, recipients, and subject (substring) and the message body (tokenized full text), ranked by relevance. Spam, trash, blocked, and unauthenticated items are always excluded. `limit` is capped at 100.
- Each search result includes an optional `highlights` object with matched fragments per field, matched terms wrapped in `**`. A field appears only when it matched.
- `GET /v0/inboxes/:inbox_id/messages` now accepts `from`, `to`, `subject` substring filters (`to` matches `to`, `cc`, or `bcc`). `GET /v0/threads`, `GET /v0/inboxes/:inbox_id/threads`, and `GET /v0/pods/:pod_id/threads` accept `senders`, `recipients`, `subject`. Filtered list requests are served by search, cap `limit` at 100, and keep newest-first ordering.

## 2026-05-28 Inbox metadata

- Inboxes support custom `metadata` key-value pairs. Values may be a string, number, or boolean, with up to 256 keys per inbox. The `Inbox` object includes an optional `metadata` field on get, list, and create responses.
- `POST /v0/inboxes` accepts `metadata` at creation. `PATCH /v0/inboxes/:inbox_id` merges `metadata`: included keys are added or overwritten, omitted keys are preserved, a key with a null value is removed, and `metadata` set to null clears everything.
- Each `PATCH /v0/inboxes/:inbox_id` update must include at least one of `display_name` or `metadata`.

## 2026-03-18 Inbox-scoped API keys

- Added `GET /v0/inboxes/:inbox_id/api-keys`, `POST /v0/inboxes/:inbox_id/api-keys`, and `DELETE /v0/inboxes/:inbox_id/api-keys/:api_key` for credentials restricted to a single inbox.
- `ApiKey` and `CreateApiKeyResponse` now include an optional `inbox_id` field when the key is scoped to an inbox.

## 2025-12-22 Webhooks and events

- Webhooks launched: HTTP callbacks for `message.received`, `message.sent`, `message.delivered`, `message.bounced` (with type and recipients), `message.complained`, `message.rejected`, and `domain.verified`.
- Verification via Svix-style headers `svix-id`, `svix-signature`, `svix-timestamp`. Filter by inbox or pod, up to 10 per webhook.
- Payloads include `inbox_id`, `thread_id`, `message_id`, timestamps, and event-specific data.

## 2025-10-28 Custom domains

- Added `GET /domains`, `GET /domains/{domain_id}`, `POST /domains`, `DELETE /domains/{domain_id}`, `GET /domains/{domain_id}/zone-file`, and `POST /domains/{domain_id}/verify`.
- DNS verification with TXT and MX records. Verification statuses: `NOT_STARTED`, `PENDING`, `VERIFYING`, `VERIFIED`, `FAILED`, `INVALID`. Per-record statuses: `MISSING`, `INVALID`, `VALID`.
- Zone file export for DNS import, plus optional per-domain feedback (bounce and complaint) delivery via `feedback_enabled`.

## 2025-10-25 Drafts API

- Added `GET /drafts`, `GET /drafts/{draft_id}`, `POST /inboxes/{inbox_id}/drafts`, `PATCH /inboxes/{inbox_id}/drafts/{draft_id}`, `POST /inboxes/{inbox_id}/drafts/{draft_id}/send`, and `DELETE /inboxes/{inbox_id}/drafts/{draft_id}`.
- Compose with `to`, `cc`, `bcc`, `subject`, plain text, HTML body, attachments, and labels. Reply threading via `in_reply_to` and `references`. Scheduled send via `send_at`.
- List and filter drafts by inbox, labels, or time range.

## 2025-08-13 Metrics endpoints

- Added `GET /metrics` (all inboxes) and `GET /inboxes/{inbox_id}/metrics` (single inbox).
- Tracks delivery events (sent, delivered, bounced, rejected) and error events (complaints, spam reports) as time-series data with timestamps.

## 2025-07-20 WebSocket streaming

- Added `wss://ws.agentmail.to/v0` for real-time event streaming: `message.received`, `message.sent`, `message.delivered`, `message.bounced`, `message.complained`.
- JWT-based authentication, event filtering by inbox, automatic reconnection with exponential backoff, typical latency under 100ms.

## 2025-06-15 Pods

- Added `POST /pods`, `GET /pods`, `GET /pods/{pod_id}`, `DELETE /pods/{pod_id}`, `POST /pods/{pod_id}/inboxes`, `POST /pods/{pod_id}/domains`, `GET /pods/{pod_id}/threads`, and `GET /pods/{pod_id}/metrics`.
- Team workspaces with shared inbox access, per-pod domain configuration, and isolated per-pod metrics.

## Related

- `/core/send` - drafts, reply and forward flow, scheduled send
- `/core/receive` - messages, list filters, inbox metadata
- `/core/conversations` - threads and thread search
- `/advanced/webhooks` - webhook setup, verification, and scoping
- `/advanced/websockets` - WebSocket streaming
- `/advanced/custom-domains` - domain verification and subdomains
- `/advanced/plans-and-usage` - metrics and usage measurement
- `/advanced/multi-tenant` - pods and API key scoping
